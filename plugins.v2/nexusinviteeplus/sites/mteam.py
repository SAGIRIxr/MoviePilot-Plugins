...
class MTeamHandler(_ISiteHandler):
    ...
    def parse_invite_page(self, site_info: Dict[str, Any], session: requests.Session) -> Dict[str, Any]:
        ...
        # 字段可能是 NULL，get(k, "") 会原样返回 None，后面拿去拼请求头会炸
        api_key = (site_info.get("apikey") or "").strip()
        # M-Team 早就改成只认 x-api-key 了，Authorization 现在是可选的
        authorization = (site_info.get("token") or "").strip()
        ...
        try:
            # 检查API认证信息：只有 API Key 是必须的
            if not api_key:
                logger.error(f"站点 {site_name} 缺少 API Key")
                result["invite_status"]["reason"] = "缺少 API Key，请在站点设置中填写"
                return result
            ...
            headers = {
                ...
                "User-Agent": (site_info.get("ua") or "Mozilla/5.0"),
                "Accept": "application/json, text/plain, */*",
                "x-api-key": api_key,
            }
            # 只在真填了 token 的时候才带 Authorization，带个空串会被 API 直接拒
            if authorization:
