...
from ..sitehttp import (REASON_COOKIE_EXPIRED, SiteAccessError, classify, get, request)
...
class NexusPhpHandler(_ISiteHandler):
    ...
    def _diagnose_no_user_id(session: requests.Session, site_url: str) -> str:
        """
        拿不到用户ID时，回头看一眼首页到底是什么状况，给一个能照着做的原因。
        """
        try:
            response = get(session, site_url, "/")
        ...
        if response.status_code >= 400:
            return f"站点返回 HTTP {response.status_code}，暂时无法访问"
        # 页面能打开、也不是登录页，那多半是这站根本不是 NexusPHP
        return (f"{REASON_COOKIE_EXPIRED}；若确认 Cookie 有效，"
                f"则该站点可能不是 NexusPHP 体系（页面上没有 userdetails.php 链接）")
    ...
    def parse_invite_page(self, site_info: Dict[str, Any], session: requests.Session) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if not user_id:
                    # 基类已经把首页/控制面板/索引页都试过了，还拿不到用户ID，
                    # 那八成不是「哪个页面不对」，而是压根没登录上。分清楚这两种，
                    # 用户才知道该去换 Cookie 还是去查站点。
                    early_failure_reason = self._diagnose_no_user_id(session, site_url)
            ...
            if not early_check_failed:
                ...
                try:
                    # 走 sitehttp 而不是裸 session.get：邀请页单独挂 CF 盾的站点
                    # （UBits 首页正常、invite.php 被拦）在这里能自动过盾重试
                    response = request(session, invite_url)

                    # 先按分类给原因：CF 盾、Cookie 失效、站点下线都各有各的说法，
                    # 全报成「访问邀请页面失败: 403 Forbidden」等于什么都没说
                    classified = classify(response)
                    if classified:
                        early_failure_reason = classified
                        logger.error(f"站点 {site_name} 检查失败: {early_failure_reason}")
                        early_check_failed = True
    ...
    def _parse_nexusphp_invite_page(self, site_name: str, html_content: str, is_next_page: bool = False) -> Dict[str, Any]:
        ...
        if not is_next_page:
            ...
            if not invite_reason and not can_invite:
                # 页面上既没有邀请表单、也没有任何提示文案，最常见的情况就是
                # 等级/权限还不够，站点干脆不渲染邀请区块
                invite_reason = "页面上没有邀请表单，通常是等级或权限未达到站点要求"
                logger.debug(f"站点 {site_name} 邀请页没有表单也没有提示文案")
