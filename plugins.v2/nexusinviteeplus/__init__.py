...
class NexusInviteePlus(_PluginBase):
    # 插件名称
    plugin_name = "后宫管理系统(自用版)"
    # 插件描述
    plugin_desc = "管理 MP 站点的邀请系统：除 NexusPHP 外额外支持 UNIT3D / Gazelle / TTG 体系，Cookie 失效与站点故障分类上报。"
    # 插件图标
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "nexusinviteeplus_"
    # 加载顺序
    plugin_order = 21
    # 可使用的用户级别
    auth_level = 2
    ...
    def __reload_modules(self):
        ...
        try:
            ...
            for module_name in list(sys.modules.keys()):
                if module_name.startswith(_PKG + '.') and module_name != _PKG:
            ...
            importlib.import_module('.data', package=_PKG)
            importlib.import_module('.utils', package=_PKG)
            importlib.import_module('.module_loader', package=_PKG)
    ...
    def get_dashboard_meta(self) -> Optional[List[Dict[str, str]]]:
        ...
        """
        return [{
            "key": "nexusinviteeplus_dashboard",
            "name": "后宫管理系统"
        }]
    ...
    def get_dashboard(self, key: str, **kwargs) -> Optional[Tuple[Dict[str, Any], Dict[str, Any], List[dict]]]:
        ...
        获取插件仪表盘页面
        """
        if key != "nexusinviteeplus_dashboard":
            return None
...
"content": [
                                ...
                                'content': [
                                    {
                                        'component': 'VSwitch',
                                        'props': {
                                            'model': 'browser_emulation',
                                            'label': '浏览器仿真过 Cloudflare',
                                            'persistent-hint': True,
                                            'hint': '站点返回 Cloudflare 挑战页时，用 MoviePilot 的 CloakBrowser 内核取一次 cf_clearance 再重试。'
                                                    '需要容器内已安装浏览器仿真环境；关闭后遇到 CF 盾会直接报错。'
                                        }
                                    }
                                ]
                            }
    ...
    @staticmethod
    def _site_failure(site_name: str, reason: str) -> Dict[str, Any]:
        """站点取数失败时的统一返回体，省得每处各拼一份结构。"""
        return {
            "error": reason,
            "invite_status": {
                "can_invite": False,
    ...
    def _pick_handler(self, site_name: str, site_url: str, session, is_mteam: bool):
        """
        挑站点处理器。

        上游只按域名硬匹配，匹配不上就一律当 NexusPHP，于是 UNIT3D、Gazelle、TTG
        这些根本没有 usercp.php 的站点全都报「无法获取用户ID」。这里先按域名找
        专用处理器，找不到就抓一次首页看指纹，实在认不出来才退回 NexusPHP。
        """
        if is_mteam:
        ...
        if handler:
            logger.info(f"站点 {site_name} 使用 {handler.__class__.__name__}")
            return handler

        schema = None
        try:
            home = sitehttp.get(session, site_url, "/")
            schema = sitehttp.detect_schema(home.text, site_url)
        except sitehttp.SiteAccessError as e:
            logger.warning(f"站点 {site_name} 探测体系时访问失败: {e.reason}")
        except Exception as e:
            logger.warning(f"站点 {site_name} 探测体系失败: {str(e)}")

        if schema and schema != "nexusphp":
            for handler_class in self._site_handlers:
                if getattr(handler_class, "site_schema", "") == schema:
                    logger.info(f"站点 {site_name} 按页面指纹识别为 {schema}，使用 {handler_class.__name__}")
                    return handler_class()
            logger.warning(f"站点 {site_name} 指纹是 {schema}，但没有对应处理器，退回 NexusPHP")
    ...
    def _get_site_invite_data(self, site, display_name: str = None):
        ...
        try:
            ...
            site_cookie = (site_info.get("cookie") or "").strip()
            ua = (site_info.get("ua") or "").strip()
            ...
            if is_mteam:
                api_key = (site_info.get("apikey") or "").strip()
                # M-Team 现在只认 x-api-key，Authorization token 早就不是必填了，
                # 上游还把它列为必填，导致只配了 apikey 的站点直接被判失败。
                token = (site_info.get("token") or "").strip()
            ...
            # 构建请求Session（UA / Cookie / 代理都在 sitehttp 里统一挂好）
            # 复制一份再塞开关，别污染 SitesHelper 缓存里的那个 dict
            site_info = dict(site_info)
            site_info["browser_emulation"] = self._browser_emulation
            session = sitehttp.build_session(site_info)
            ...
            if is_mteam:
                ...
                try:
                    test_response = sitehttp.get(session, site_url, "/")
                except sitehttp.SiteAccessError as e:
                    return self._site_failure(site_name, e.reason)
                if test_response.status_code >= 400:
                    return self._site_failure(
                        site_name,
                        f"API认证失败，请检查 API Key 是否有效，状态码: {test_response.status_code}")
            else:
                # 连通性预检。注意不能只看首页：UBits 首页挂着 CF 盾、内页却完全正常，
                # 上游拿首页的 403 一票否决，等于把能用的站点全判死。
                reason = sitehttp.check_alive(session, site_info)
                if reason:
                    logger.error(f"站点 {site_name} 预检失败: {reason}")
                    return self._site_failure(site_name, reason)

            # 使用站点处理器
            logger.info(f"站点 {site_name} 开始处理邀请数据")

            handler = self._pick_handler(site_name, site_url, session, is_mteam)

            # 使用处理器解析邀请页面
            site_data = handler.parse_invite_page(site_info, session)

            # 各 handler 的列映射难免猜偏，出来之后统一洗一遍：
            ...
            # 已经归好类的网络故障（域名没了、证书、超时），原样报出去就行，
            # 不要再套一层「获取站点邀请数据失败: HTTPSConnectionPool(...)」的噪音
            logger.error(f"站点 {site_name} 访问失败: {e.reason}")
            return self._site_failure(site_name, e.reason)
        except Exception as e:
            logger.error(f"获取站点 {site_name} 邀请数据失败: {str(e)}")
            logger.debug(traceback.format_exc())
            return self._site_failure(site_name, f"获取站点邀请数据失败: {str(e)}")
            ...
            for site in selected_sites:
                ...
                if "error" in site_data:
                    ...
                    failure_indicators = [
                        ...
                        # sitehttp 分好类的故障也算失败，否则它们会被当成「刷新成功但没邀请」
                        r"Cookie\s*已失效",
                        r"找不到当前登录用户",
                        r"Cloudflare\s*盾",
                        r"域名无法解析",
    ...
    def get_api_handlers():
        ...
        """
        return {
            "/get_invitees": {"func": NexusInviteePlus.get_invitees, "methods": ["GET"], "desc": "获取所有站点邀请数据"},
            "/refresh": {"func": NexusInviteePlus.refresh_data, "methods": ["GET"], "desc": "强制刷新站点数据"}
        }
