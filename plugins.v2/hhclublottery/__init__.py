...
class HHClubLottery(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        if picked:
            ...
            if self._running:
                ...
                names = "、".join(name for _, _, name, _ in picked)
                ok, icon, title, message = False, "⛔", "操作未执行", (
                    f"正在抽奖，{names} 都不能做 —— 这一轮收尾时会把统计整份写回，"
                    "现在改的会被盖掉。等跑完，或先点「停止」。")
            elif len(picked) > 1:
                names = "、".join(name for _, _, name, _ in picked)
                ok, icon, title, message = False, "⛔", "操作未执行",                     f"{names} 同时勾上了，一次只能做一件，都没执行"
            else:
                key, icon, title, runner = picked[0]
                ok, message = runner()
                if ok and key == "do_import":
                    self._import_data = ""   # 导完清空，免得下次保存又导一遍

            (logger.info if ok else logger.error)(f"{icon} {title}：{message}")
            if self._notify:
                self.post_message(mtype=NotificationType.SiteMessage,
                                  title=f"【HHCLUB 幸运大转盘】{title}",
    ...
    def api_run(self) -> dict:
        ...
        if not self._enabled:
            return {"code": 1, "message": "插件未启用，先到配置页勾上「启用插件」"}
        if self._running:
        ...
        if running and live and not warming_up:
            ...
            body = [
                {"component": "div", "props": {"class": "text-subtitle-1 font-weight-bold"},
                 "text": ("正在启动 · 读取余额与单抽消耗…" if warming_up or live
                          else "正在抽奖 · 这一下没读到进度")},
