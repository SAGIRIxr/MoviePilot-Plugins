...
```
NS签到-mp版/
├── package.v2.json                 # 插件仓库清单（添加为插件库时使用）
└── plugins.v2/
    └── nodeseeksignin/
        ├── __init__.py             # 插件主体
...
1. 将本项目上传到一个 GitHub 仓库（仓库根目录需包含 `package.v2.json` 与 `plugins.v2/`）。
2. MoviePilot → 设定 → 插件 → 插件仓库，添加你的仓库地址：
   `https://github.com/<用户名>/<仓库名>`
3. 在「插件市场」中找到 **NodeSeek签到** 并安装。

### 方式二：本地直接放置

将 `plugins.v2/nodeseeksignin` 整个目录复制到 MoviePilot 的插件目录
（容器内通常为 `/app/app/plugins/`，即 `app/plugins/nodeseeksignin/`），重启 MoviePilot 后即可在插件列表看到。

> 首次启用插件时，MoviePilot 会自动安装 `requirements.txt` 中的 `curl_cffi`。
